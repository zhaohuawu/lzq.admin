...
export default {
  name: 'SysUserList',
  components: { OperateTable, Pagination, ImageCropper },
  data() {
    return {
      list: null,
      tableCols: [
        { label: '状态', prop: 'statusName', align: 'center', width: '60' },
        { label: '头像', prop: 'headImgLink', type: 'Image', align: 'center', width: '120', style: 'height: 100px' },
        { label: '登录账号', prop: 'loginName', align: 'center', width: '100' },
        { label: '登录账号', prop: 'userName', align: 'center', width: '100' },
  ...
  methods: {
    ...
    operationSelect(operationName, row) {
      if (operationName === 'Edit') {
        window.open(`#/designer?id=${row.id}`, '_blank')
